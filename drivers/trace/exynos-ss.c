...
#define S3C_ADDR_BASE   (VMALLOC_START + 0xF6000000)
...
#define S5P_VA_SS_BASE			phys_to_virt(0x26000000)
...
static void sec_debug_set_upload_cause(enum sec_debug_upload_cause_t type)
{
	...
	pmu_raw_writel(type, 0x080C);
	pmu_raw_writel(type, 0x0810);
	pmu_raw_writel(type, 0x0818);
}

#define EXYNOS5_PA_SDRAM      0x20000000
...
void exynos_ss_set_upload(void)
{
	...
	pr_emerg("set upload cause..");
}
EXPORT_SYMBOL(exynos_ss_set_upload);
...
static unsigned int exynos_ss_get_core_power_stat(unsigned cpu)
{
...
int exynos_ss_prepare_panic(void)
{
	...
	for (cpu = 0; cpu < ESS_NR_CPUS; cpu++) {
		if (exynos_ss_get_core_power_stat(cpu))
...
int exynos_ss_save_context(void *v_regs)
{
	...
	pr_emerg("1. exynos-snapshot: %s\n", __func__);

	local_irq_save(flags);
	pr_emerg("2. exynos-snapshot: %s\n", __func__);
	...
	if (exynos_ss_get_core_panic_stat(smp_processor_id()) !=  ESS_SIGN_PANIC) {
		...
		pr_emerg("exynos-snapshot: skip context saved(CPU:%d)\n",
							smp_processor_id());
#endif
	pr_emerg("2.5. exynos-snapshot: %s %d \n", __func__, smp_processor_id());

	exynos_ss_save_mmu(per_cpu(ess_mmu_reg, smp_processor_id()));
	pr_emerg("3. exynos-snapshot: %s\n", __func__);

	exynos_ss_save_reg(regs);
	pr_emerg("4. exynos-snapshot: %s\n", __func__);

	exynos_ss_dump();
	pr_emerg("5. exynos-snapshot: %s\n", __func__);
...
static unsigned int __init exynos_ss_remap(unsigned int base, unsigned int size)
{
	...
	for (i = 0; i < ARRAY_SIZE(ess_items); i++) {
		...
		if (i == 0) {
			...
			ess_items[i].entry.vaddr = (size_t)phys_to_virt(base);
...
static int __init exynos_ss_setup(char *str)
{
	...
	pr_info("exynos-snapshot: try to reserve dedicated memory : 0x%lx, 0x%zx\n",
			base, size);
...
out:
	pr_err("exynos-snapshot: buffer reserved failed : 0x%lx, 0x%zx\n", base, size);
	return -1;
}
